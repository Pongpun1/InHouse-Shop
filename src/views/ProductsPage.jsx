...
import { Search, ShoppingCart, Star, SlidersHorizontal, ArrowUpDown, Loader2 } from "lucide-react";

const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3000';

const categories = ['ทั้งหมด', 'เฟอร์นิเจอร์', 'ของตกแต่ง', 'โคมไฟ', 'สิ่งทอและพรม', 'ต้นไม้ประดับ', 'เครื่องหอม', 'โต๊ะอาหาร'];
...
export default function ProductsPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "rating-desc": return result.sort((a, b) => (b.rating || 0) - (a.rating || 0));
    ...
    <div className="min-h-screen bg-stone-50 pb-20 font-sans">
      ...
      <div className="bg-stone-900 pt-10 pb-16 px-4">
        <div className="max-w-7xl mx-auto text-center">
          <h1 className="text-3xl sm:text-4xl font-extrabold text-white mb-4">สินค้าทั้งหมด</h1>
          <p className="text-stone-400">ค้นหาไอเทมตกแต่งบ้านที่สะท้อนความเป็นคุณ</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 -mt-8">
        ...
        <div className="mb-6 text-stone-500 text-sm">
          พบสินค้าทั้งหมด <span className="font-bold text-stone-900">{filteredProducts.length}</span> รายการ
        </div>
        ...
        {loading ? (
            ...
            {filteredProducts.map((product) => (
              ...
              >
                <div className="relative h-60 overflow-hidden bg-stone-100">
                  {product.imageUrl ? (
                    <img
                      src={`${API_URL}${product.imageUrl}`} // รูปมาจาก Express static /uploads/
                      alt={product.name}
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
                    />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-stone-300 text-sm">ไม่มีรูปภาพ</div>
                  )}
                ...
                <div className="p-5 flex flex-col flex-grow">
                  ...
                  <div className="mt-auto pt-4 flex items-center justify-between">
                    <p className="text-xl font-black text-stone-900">฿{Number(product.price).toLocaleString()}</p>
                    <button
                      ...
                      title="หยิบใส่ตะกร้า"
                      onClick={() => console.log('เพิ่มตะกร้า:', product.name)} // TODO: ต่อ cart API
                    >
          ...
          <div className="text-center py-20 bg-white rounded-3xl border border-stone-200 border-dashed mt-4">
            ...
            <h3 className="text-lg font-bold text-stone-900 mb-1">ไม่พบสินค้าที่คุณค้นหา</h3>
            <p className="text-stone-500">ลองเปลี่ยนคำค้นหา หรือเลือกหมวดหมู่ใหม่</p>
            <button onClick={() => { setSearchTerm(''); setSelectedCategory('ทั้งหมด'); }}
